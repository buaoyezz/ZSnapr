import os
import flet as ft
from config import DEFAULT_SETTINGS, SUPPORTED_FORMATS, HOTKEYS, get_available_locales, get_current_language, set_language

# 使用健壮的翻译助手
try:
    from translation_helper import t
    print(f"[{os.path.basename(__file__)}] Using translation_helper")
except ImportError:
    # 回退到config
    try:
        from config import t
        print(f"[{os.path.basename(__file__)}] Using config translation")
    except ImportError:
        # 最终回退
        def t(key, **kwargs):
            print(f"[{os.path.basename(__file__)}] Translation fallback: {key}")
            return key

def _on_language_change(app, e):
    # Handle language change
    try:
        new_language = e.control.value
        # Use the app's change_language method which handles UI refresh properly
        app.change_language(new_language)
    except Exception as ex:
        app._update_status(f"Language change error: {str(ex)}", ft.Colors.RED)

def build(app):
    app.save_dir_field = ft.TextField(
        label=t("settings.save_directory"),
    ...
    # Add auto option
    language_options = [ft.dropdown.Option("auto", "Auto-detect")]
    language_options.extend([ft.dropdown.Option(code, name) for code, name in available_locales.items()])
    
    app.language_dropdown = ft.Dropdown(
        label=t("settings.language"),
        value=current_language if current_language in available_locales else "auto",
        options=language_options,
        width=160,
        border_radius=8,
        filled=True,
        bgcolor=ft.Colors.GREY_50,
        on_change=lambda e: _on_language_change(app, e)
    )
    ...
    app.auto_copy_fullscreen_checkbox = ft.Checkbox(
        label=t("settings.auto_copy_fullscreen"),
    ...
    app.auto_copy_window_checkbox = ft.Checkbox(
        label=t("settings.auto_copy_window"),
    ...
    app.fullscreen_hotkey_field = ft.TextField(
        label=t("settings.fullscreen_hotkey"),
    ...
    fullscreen_record_btn = ft.IconButton(
        ...
        tooltip=t("settings.record_hotkey"),
    ...
    app.region_hotkey_field = ft.TextField(
        label=t("settings.region_hotkey"),
    ...
    app.window_hotkey_field = ft.TextField(
        label=t("settings.window_hotkey"),
            ...
            ft.Container(
                content=ft.Column([
                    ft.Row([
                        ...
                        ft.Text(t("settings.title"), size=16, weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_800)
                    ], spacing=10),
                    ft.Divider(height=1, color=ft.Colors.BLUE_100, thickness=1),
                    ft.Container(
                        content=ft.Column([
                            ft.Text(t("settings.language"), size=12, weight=ft.FontWeight.W_500, color=ft.Colors.GREY_700),
                            app.language_dropdown
                        ], spacing=5),
                        margin=ft.margin.symmetric(vertical=8)
                    ),
                    ft.Container(
                        content=ft.Column([
                            ft.Text(t("settings.save_directory"), size=12, weight=ft.FontWeight.W_500, color=ft.Colors.GREY_700),
                    ...
                    ft.Container(
                        content=ft.Row([
                            ft.Container(
                                content=ft.Column([
                                    ft.Text(t("settings.image_format"), size=12, weight=ft.FontWeight.W_500, color=ft.Colors.GREY_700),
                                    app.format_dropdown
                                ], spacing=5),
                                expand=1
                            ),
                            ...
                            ft.Container(
                                content=ft.Column([
                                    ft.Text(t("settings.delay"), size=12, weight=ft.FontWeight.W_500, color=ft.Colors.GREY_700),
                                    app.delay_field
                                ], spacing=5),
                                expand=1
                            )
            ...
            ft.Container(
                content=ft.Column([
                    ft.Row([
                        ...
                        ft.Text(t("settings.hotkeys"), size=16, weight=ft.FontWeight.BOLD, color=ft.Colors.PURPLE_800)
                    ], spacing=10),
                    ft.Divider(height=1, color=ft.Colors.PURPLE_100, thickness=1),
                    ft.Container(
                        content=ft.Column([
                            ft.Text(t("settings.hotkeys"), size=12, weight=ft.FontWeight.W_500, color=ft.Colors.GREY_700),
                            ft.ResponsiveRow([
